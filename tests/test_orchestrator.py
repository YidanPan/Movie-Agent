...
class MovieOrchestratorTests(unittest.TestCase):
    def test_creates_a_structured_project(self) -> None:
        with TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(approved.status, "final_cut_approved")
            self.assertEqual(approved.subtitle_mode, "soft")
            self.assertIsNone(approved.final_output_placeholder)
            completed = MovieOrchestrator(settings).generate_final_master(project.project_id)
            self.assertEqual(completed.status, "completed_mock")
            self.assertEqual(completed.final_output_placeholder, f"outputs/{project.project_id}/final-cut.mp4")
